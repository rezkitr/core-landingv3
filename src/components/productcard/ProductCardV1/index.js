...
const ProductCardV1 = ({ isPromo = false, className = '' }) => {
  return (
    <div
      className={`product-card relative h-64 w-[140px] max-w-[140px] overflow-hidden rounded-xl md:h-72 md:w-[200px] md:max-w-[200px] ${className}`}
    >
      <div className="h-full">
        <div
          className={`relative ${
            isPromo ? 'h-[150px] md:h-52' : 'h-44 md:h-56'
          }`}
        >
          <div className="absolute inset-0 z-10 bg-gradient-to-t from-white/50" />
          <Image
      ...
      <div className="absolute inset-x-0 bottom-0 z-10">
        <div className="absolute -top-4 right-0 h-6 w-2/5 rounded-t-lg bg-primaryBlack md:-top-6 md:rounded-t-xl">
